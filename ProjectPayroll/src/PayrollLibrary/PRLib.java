...
public class PRLib {
    ...
    public static float convAndRoundTime(String time){      // takes the string input time and converts it to a float value
    ...
    public static int getHours(String time){          // method to get Hours as a byte
        String[] splitFields = time.split(":");
        return Integer.parseInt(splitFields[0]);
    }
    
    public static int getMinutes(String time){            // method to get minutes as byte
        String[] splitFields = time.split(":");
        return Integer.parseInt(splitFields[1]);
    }
    ...
    public static float calcWeeklyHrsWorked(float elapsedTimes[]){          //this method calculates how many hours are worked in a week
        float sumHours = 0;
        for (int a = 0; a<7; a++)
    ...
    public static float calcRegHrsWorked(float weeklyHours){                //this method returns weekly hours worked
        if (weeklyHours>40){
            return 40f;
        }
        return weeklyHours;
    ...
    public static float calcOTHrs(float weeklyHours){                       // this method returns OT hours worked
        if (weeklyHours>40){
            return weeklyHours;
        }
                return 0;
    ...
    public static float calcWeekendHrs(float clockIn, float clockOut, int day){             //this method will calculate the weekend hours worked
        float weekendHours = 0;
        if (day == 4 && (clockOut<clockIn)){
...
public static float calcPay(float hoursWorked, float hourlyPay, float shiftRate){   //method to calculate pay using 3 params
    
    if (hoursWorked>=0 && hourlyPay>0 && shiftRate>0)    // again make sure they're greater than or equal to 0
        return Round((hoursWorked*hourlyPay)*shiftRate); // return pay times rate multiplier
    return 0; 
}
...
public static float fwTax(float Pay, char mStatus, int numExemptions){
    if (Pay>=0){
        Pay = Pay - (numExemptions*155.80f);
        if (mStatus=='S'){
            if (Pay>0 && Pay<=88){
                return 0;
...
public static ShiftRates get2ndShiftRate(ArrayList<ShiftRates> arrayShiftList){         //method that returns the 2nd shift pay multiplier
    ShiftRates get2nd = new ShiftRates(); 
           for(ShiftRates rate:arrayShiftList){
                if(rate.getshiftCode()=='2'){
                    get2nd=rate;
                }
           }
    return get2nd;
...
public static ShiftRates get3rdShiftRate(ArrayList<ShiftRates> arrayShiftList){         //method returns the 3rd shift pay multiplier
    ShiftRates get3rd = new ShiftRates(); 
           for(ShiftRates rate:arrayShiftList){
                if(rate.getshiftCode()=='3'){
                    get3rd=rate;
                }
           }
    return get3rd;
...
public static ShiftRates getweekendShiftRate(ArrayList<ShiftRates> arrayShiftList){         //method returns the weekend shift multiplier
    ShiftRates getWeekend = new ShiftRates(); 
           for(ShiftRates rate:arrayShiftList){
                if(rate.getshiftCode()=='W'){
                    getWeekend=rate;
                }
           }
    return getWeekend;
